...
[IMAGE 01] Prefill predicts “blue”; feeding it back creates its K/V before its query reads all four positions and predicts “.”. One layer is shown; “Rest of model” includes the remaining layers and vocabulary projection. Token pieces and vector glyphs are illustrative; only the final prompt query is shown.
...
[IMAGE 02] Single-head attention: compare the current query with keys through position t, scale by the square root of d, apply softmax, then combine the corresponding values.
...
[IMAGE 03] Appending E preserves cached A–D. Replacing B with X preserves A but requires recomputing the dependent suffix. Colored vector glyphs show cache identity and validity, not measured activations; recomputation need not change every numerical value.
...
[IMAGE 04] New h → Q/K/V projections → append new K/V → Q reads past and current keys → softmax weights → weighted V sum. Earlier K/V stay fixed. The weight bars use a calculated toy example, not measured model activations; positional operations and the rest of the model are omitted.
...
[IMAGE 05] Separate K/V payload in bytes: two tensors × batch size × layers × retained positions × KV heads × head width × bytes per element.
