...
| `CAND-ANIM-008` | Agent, automatic math trigger | Chapter 4 | Input states → learned `Q`, `K`, and `V` projections → scaled query-key score matrix → causal mask → row-wise attention distributions → weighted value retrieval; contrast broken scaling and broken masking | discuss | Derive and verify the mechanism during Day 4; production only on the Mac Studio after explicit approval |
| `CAND-ANIM-009` | Agent, automatic math trigger from learner inference | Chapter 4–modern decoder bridge | Contrast the same token in two contexts to establish distinct request-local K/V states; during prefill, use all prompt queries once but retain prompt keys and values; during decoding, each transient new query reads the unchanged-prefix cache, then its new key/value pair is appended | discuss | Verify cache shapes and cached/uncached equivalence; prevent any suggestion of a global per-token cache; production only on the Mac Studio after explicit approval |
