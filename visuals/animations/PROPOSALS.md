...
| `CAND-ANIM-009` | Agent, automatic math trigger from learner inference | Chapter 4–modern decoder bridge | During prefill, use all prompt queries once but retain prompt keys and values; during decoding, each transient new query reads the growing persistent KV tracks, then its new key/value pair is appended | discuss | Verify cache shapes and cached/uncached equivalence before production; production only on the Mac Studio after explicit approval |
