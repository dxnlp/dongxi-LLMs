...
class LoopedTransformer(Scene):
    def construct(self):
        ...
        title=txt('同一组权重，多次计算',34,[-3.9,3.38],cjk=True)
        ...
        block=SVGMobject(str(P/'assets/block.svg')).stretch_to_fit_width(2.96).stretch_to_fit_height(2.59).move_to([0,.05,0])
        heading=txt('Transformer',27,[0,1.10])
        ...
        caption=txt('一组参数，一个计算模块',27,[0,-1.89],cjk=True)
        def explain(s):
            nonlocal caption
            new=txt(s,27,[0,-1.89],cjk=True)
            self.play(FadeOut(caption,shift=DOWN*.04),FadeIn(new,shift=UP*.04),run_time=.4)
            caption=new
        self.play(FadeIn(caption),run_time=.4)
        counter_labels=VGroup(*[txt(s,24,[x,-2.66],MUTED,cjk=True) for x,s in zip((-4.35,0,4.35),('存储参数','模块执行次数','矩阵运算量'))])
        ...
        loop_ink=SVGMobject(str(P/'assets/loop.svg')).stretch_to_fit_width(11.9).stretch_to_fit_height(2.27).move_to([0,1.215,0])

        for r in range(1,4):
            explain(('隐藏状态进入模块','新的隐藏状态，再经过同一模块','第三次计算，仍使用同一组权重')[r-1])
        ...
        caveat=txt('额外计算不保证更好的预测',20,[0,-3.81],MUTED,cjk=True)
        self.play(FadeIn(caveat),run_time=.6)
        self.wait(3);mark('final')
        (P/'timeline.json').write_text(json.dumps(dict(checkpoints=checkpoints,duration=float(self.time),geometry_checks=True),indent=2)+'\n')
